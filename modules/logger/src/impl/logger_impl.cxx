...
namespace stdx
{
    namespace detail
    {
        ...
        void LoggerImpl::worker_thread_function()
        {
            while (is_running_)
            {
...

                {
                    ...
                    while (!message_queue_.empty() && batch.size() < batch_size_)
                    {
                ...
                // Ensure force flush is processed
                if (force_flush_)
                {
                    std::lock_guard<std::mutex> file_lock(file_mutex_);
                    file_.flush();
        ...
        void LoggerImpl::rotate_file()
        {
            ...
            auto rotated_name = file_path_.stem().string() + "-" + first_timestamp_ + "-" + last_timestamp_ + file_path_.extension().string();
